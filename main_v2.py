...
def get_prices(nodename, startdate, enddate):
    ...
    for _ in range(retry_count):
        try:
            ...
            if isinstance(e, requests.exceptions.Timeout) or isinstance(e, requests.HTTPError) and e.response.status_code == 429:
                time.sleep(retry_delay)
            else:
                logging.error(f"request for {startdate}-{enddate} {nodename} in market_run_id=RTM: Error occurred: {str(e)}")
                return None
            ...
            logging.error(f"request for {startdate}-{enddate} {nodename} in market_run_id=RTM: Unhandled exception occurred: {str(e)}")
            return None
        else:
            return df[["INTERVALSTARTTIME_GMT", "INTERVALENDTIME_GMT", "NODE", "MARKET_RUN_ID", "LMP_TYPE", "MW", "OPR_INTERVAL"]]
...
if __name__ == '__main__':
    ...
    for node_name in names_lst:
        df_rtm = pd.DataFrame(columns=["INTERVALSTARTTIME_GMT", "INTERVALENDTIME_GMT", "NODE", "MARKET_RUN_ID", "LMP_TYPE", "MW", "OPR_INTERVAL"])
        df_dam = pd.DataFrame(columns=["INTERVALSTARTTIME_GMT", "INTERVALENDTIME_GMT", "NODE", "MARKET_RUN_ID", "LMP_TYPE", "MW"])
        for i in tqdm(range(len(dates_list)-1)):
