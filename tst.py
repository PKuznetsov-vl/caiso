...
def request_file():
    ...
    all_months_df['UTC timestamp'] = pd.to_datetime(all_months_df['UTC timestamp'], format='%Y-%m-%d %H:%M:%S',
                                                    utc=True)
    # add PST column
    ...
    # Write to CSV
    month = 'all_months'
    all_months_df.to_csv('OASIS_Scrape_OutputV2/' + node + '_' + str(year) + '_' + queryname + '.csv',
                         columns=list(all_months_df))
